...
const Trending = () => {
  ...
  const trendingMovies = toggleDayWeek === "day" ? state.trending || [] : state.week || [];
  ...
  const handleLoadMore = () => {
    setVisibleCards((prev) => Math.min(prev + 5, trendingMovies.length));
  };
    ...
    <div className="flex flex-col items-center w-full">
      <div className="flex justify-between w-full items-center">
        ...
        <div className="flex space-x-2 mr-5">
          <button
            className={`px-4 py-2 rounded ${toggleDayWeek === "day" ? "bg-orange-500 text-white" : "bg-gray-200 text-black"}`}
            onClick={() => setToggleDayWeek("day")}
          >
            Day
          </button>
          <button
            className={`px-4 py-2 rounded ${toggleDayWeek === "week" ? "bg-orange-500 text-white" : "bg-gray-200 text-black"}`}
            onClick={() => setToggleDayWeek("week")}
          >
            Week
          </button>
        </div>
      ...
      <div className="flex items-center justify-center mb-4">
        <i className="fa-solid fa-angle-left cursor-pointer text-blue-500 text-2xl hover:text-gray-300 transition duration-200" onClick={handlePrev}></i>
      </div>
        ...
        {trendingMovies.slice(visibleCards - 5, visibleCards).map((val) => (
          <div key={val.id} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/5 p-2 transition-transform transform duration-700 ease-in-out">
            <div className="bg-[#04152d] text-white rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105">
              <img src={`${imageUrl}${val.poster_path}`} alt={val.title} className="w-full h-48 object-cover" />
              <div className="p-4">
      ...
      <div className="flex items-center justify-center mt-4">
        <i className="fa-solid fa-angle-right cursor-pointer text-blue-500 text-2xl hover:text-gray-300 transition duration-200" onClick={handleLoadMore}></i>
      </div>
    </div>
  );
};
